...
void main() {
    ...
    float factor = clamp(line + glow, 0.0, 1.0);
    
    // Sleek dark theme background base color
    vec3 base_color = vec3(14.0/255.0, 12.0/255.0, 18.0/255.0);
    
    // Smoothly blend the wave color over the dark base background
    vec3 finalColor = mix(base_color, color, factor);
    
    // The wave and the background now uniformly share the user's opacity setting
...
class ShaderWavesVeil(VeilBase):
    def __init__(self):
        ...
        self._t = 0.0
        self._timer = QTimer()
        self._timer.setInterval(33)  # 30 FPS target drastically drops CPU overhead
    ...
    def paint(self, painter, full_rect, selection_rects, opacity, color_hex, selection_shape="rectangle"):
        ...
        if not self._program:
            fallback_color = QColor(color_hex)
            fallback_color.setAlpha(int(opacity * 255))
            painter.fillRect(full_rect, fallback_color)
        ...
        self._program.setUniformValue("opacity", float(opacity))
