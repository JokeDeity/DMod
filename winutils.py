...
# Used by get_foreground_window_info and toggle_desktop_icons
...
def set_window_topmost(hwnd):
    """Restores the HWND_TOPMOST (-1) flag stripped by window stacking."""
    user32.SetWindowPos(hwnd, -1, 0, 0, 0, 0, 0x0002 | 0x0001 | 0x0010)

def set_window_z_order(hwnd_target, hwnd_below):
    """
    Places hwnd_target directly behind hwnd_below.
    Flags:
    0x0002 = SWP_NOMOVE
    0x0001 = SWP_NOSIZE
    0x0010 = SWP_NOACTIVATE
    """
    user32.SetWindowPos(hwnd_target, hwnd_below, 0, 0, 0, 0, 0x0002 | 0x0001 | 0x0010)
...
def get_idle_time_ms():
    """Milliseconds elapsed since the last keyboard or mouse input (GetLastInputInfo)."""
    lii = _LASTINPUTINFO()
    lii.cbSize = ctypes.sizeof(lii)
    user32.GetLastInputInfo(ctypes.byref(lii))
    return ctypes.windll.kernel32.GetTickCount() - lii.dwTime

# ── Foreground window info (for Auto-Dim) ──────────────────────────────────────

# Classes that should NOT trigger a hole update — window switchers, shell chrome, etc.
AUTODIM_EXEMPT_CLASSES = frozenset({
    "XamlExplorerHostIslandWindow",   # Alt+Tab switcher (Win10/11)
...
def get_foreground_window_info():
    """
    Returns (hwnd, class_name, left, top, width, height).
    Caller should check hwnd != 0 and width/height > 0 before using.
    """
    ...
    if not user32.GetWindowRect(hwnd, ctypes.byref(rect)):
        return hwnd, class_name, 0, 0, 0, 0

    return hwnd, class_name, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top
